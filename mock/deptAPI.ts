...
export default {
  'GET /api/v1/dept/bizUnit/:bizUnitId': (req: any, res: any) => {
    const filterDepts = depts.filter(item => item.bizUnitId === parseInt(req.params.bizUnitId))
    res.json({
      data: filterDepts,
      code: 200,
      message: '查询成功'
  ...
  'POST /api/v1/dept/bizUnit/:bizUnitId': (req: any, res: any) => {
    ...
    depts.push({...req.body, id: insertId, bizUnitId: parseInt(req.params.bizUnitId)})
    res.json({
      code: 200,
      message: '新增成功',
      data: {
  ...
  'PUT /api/v1/dept/:id': (req: any, res: any) => {
    depts = depts.map(item => (item.id === parseInt(req.params.id) ? {id:parseInt(req.params.id),...req.body} : item))
    res.json({
      code: 200,
      message: '更新成功'
    });
  },
  'DELETE /api/v1/dept/:id': (req: any, res: any) => {
  ...
  'GET /api/v1/dept/bizUnitId/:bizUnitId': (req: any, res: any) => {
    depts = depts.filter(item => item.bizUnitId === parseInt(req.params.bizUnitId))
    res.json({
      data: depts,
      code: 200,
      message: '查询成功'
