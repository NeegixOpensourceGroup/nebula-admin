...
export default {
  ...
  'GET /api/v1/dictItem/:dictId': (req: any, res: any) => {
    ...
    const list = dictItem.filter(item=>item.dictId===parseInt(req.params.dictId)).filter(item=>{
  ...
  'DELETE /api/v1/dictItem/:dictId/:id': (req: any, res: any) => {
    const {dictId, id} = req.params;
    dictItem = dictItem.filter(item => !(item.id === parseInt(id) && item.dictId === parseInt(dictId)));
    res.json({
      code: 200,
      message: '删除成功'
    });
